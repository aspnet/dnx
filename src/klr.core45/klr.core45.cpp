...
typedef HRESULT (STDMETHODCALLTYPE *HostMainAsync)(
	const int argc,
	const wchar_t** argv,
	LPVOID* ppAsyncAwaiter
	);
...
extern "C" __declspec(dllexport) HRESULT __stdcall CallApplicationMainAsync(PCALL_APPLICATION_MAIN_DATA data, LPVOID* ppAsyncAwaiter)
{
	HRESULT hr = S_OK;
	FnGetCLRRuntimeHost pfnGetCLRRuntimeHost = nullptr;
	ICLRRuntimeHost2* pCLRRuntimeHost = nullptr;
	...
	LPCWSTR szAssemblyName = L"klr.core45.managed, Version=1.0.0.0";
	LPCWSTR szEntryPointTypeName = L"DomainManager";
	LPCWSTR szMainMethodName = L"ExecuteAsync";
	...
	// Call main
	hr = pHostMain(data->argc, data->argv, ppAsyncAwaiter);
	return hr;
}
